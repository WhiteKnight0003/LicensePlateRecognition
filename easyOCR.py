...
model = YOLO("save_models/license_plate_best.pt") # fine-tune
...
# Mapping to closest num of alpha
def correct_plate_format(orc_text):
    mapping_num_to_alpha = {"0":"O", "1":"I", "5":"S", "8":"B"}
    mapping_alpha_to_num = {"O":"0", "I":"1","Z":"2", "S":"5", "B":"8"}

    orc_text = orc_text.upper().replace(" ", "")
    if len(orc_text) != 7:
        return "" # Discard if wrong length
    
    corrected = []
    for i, ch in enumerate(orc_text):
        if i <2 or i>=4: # alphabet positions
            if ch.isdigit() and ch in mapping_num_to_alpha: # if num
                corrected.append(mapping_num_to_alpha[ch])
            elif ch.isalpha():
                corrected.append(ch)
        else: # numeruic positions
...
plate_final = {} # final plate per box
...
output_video = "vehicle_video_output.mp4"
...
while cap.isOpened():
    ...
    for r in results:
        boxes = r.boxes
        for box in boxes:
            conf = float(box.conf.cpu().numpy()[0]) if box.conf.numel() > 0 else 0.0
            if conf < CONF_THRESH:
                continue
            ...
            if plate_crop.size > 0:
                ...
                if oy2 <= frame.shape[0] and ox2 <= frame.shape[1]:
                    ...
                    if stable_text:
                        cv2.putText(frame, stable_text, (ox1, oy1 - 20), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 6) # black outline
                        cv2.putText(frame, stable_text, (ox1, oy1 - 20), cv2.FONT_HERSHEY_SIMPLEX, 2, (255,255,255), 3) # white text
